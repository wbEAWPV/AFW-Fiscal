...
foreach prod of local produits {
	...
	replace import = ${vatimport_`prod'} if codpr==`prod'
}
...
foreach prod of global products {
	...
	qui replace import 	 = ${vatimport_`prod'} if codpr==`prod' in `i'
	local i=`i'+1
}
...
replace informal_purchase = 0 if import ==1 
...
gen achats_net_custom = achats_net_excise / (1 + custom)
